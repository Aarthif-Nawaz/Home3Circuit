...
if(!empty($fname) && !empty($lname) && !empty($address) && !empty($postCode) && !empty($telNo) && !empty($email) && !empty($pass)){
    if($pass!=$Confirm){
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $insertSQL = "INSERT INTO Users(userType, userFname, userSname, userAddress, userPostCode, userTelNo, userEmail, userPassword) VALUES (0,'$fname','$lname','$address','$postCode','$telNo','$email','$pass')";
            $exeSQL = mysqli_query($conn, $insertSQL);
            
            if(mysqli_errno($conn)==0){
                echo "Your Registration was Successful ! <br>";
                echo "Continue to login<a href='index.php'>Login</a>";
            }
            else{
